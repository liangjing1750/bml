...
test('左侧目录会显示带标签的顶层统计摘要', async ({ page, request }) => {
  ...
  await expect(page.locator('[data-section="process"]')).toContainText('子域 2');
  await expect(page.locator('[data-section="process"]')).toContainText('流程 2');
  await expect(page.locator('[data-section="process"]')).toContainText('步骤 2');
